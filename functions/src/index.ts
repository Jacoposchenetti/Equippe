...
/**
 * Trigger su aggiornamento waitlist: gestisce la re-iscrizione di un utente disiscriitto.
 * Se il documento passa da unsubscribed=true a unsubscribed non presente (re-iscrizione),
 * invia l'email di conferma e la notifica admin — esattamente come onCreate.
 */
export const onWaitlistReSubscribe = functions
  .region('europe-west1')
  .firestore
  .document('waitlist/{docId}')
  .onUpdate(async (change) => {
    ...
    // Attiva solo quando l'utente era disiscriitto e ora non lo è più (re-iscrizione)
    if (!before.unsubscribed || after.unsubscribed) return null;

    const { nome, cognome, email, professione, citta, telefono } = after;

    // Email di conferma all'utente
    await sendEmail(
      email,
      'Bentornato/a nella waiting list di Tuaequipe.it! 🎉',
      `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #0066cc;">Ciao ${nome}! 👋</h2>
          <p>Sei di nuovo nella waiting list di <strong>Tuaequipe.it</strong>. Bentornato/a!</p>
          <p>Ti ricontatteremo tra i primi quando apriremo nella tua zona.</p>
          <p style="color: #666; font-size: 14px; margin-top: 24px;">
            I tuoi dati:<br>
            <strong>${nome} ${cognome}</strong><br>
            ${professione} — ${citta}
    ...
    for (const adminEmail of ADMIN_EMAILS) {
      await sendEmail(
        adminEmail,
        `🔄 Re-iscrizione waiting list: ${nome} ${cognome}`,
        `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #0066cc;">Re-iscrizione alla waiting list</h2>
            <p style="color: #888; font-size: 13px;">Questo utente si era precedentemente disiscriitto.</p>
            <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
              <tr><td style="padding: 8px; border-bottom: 1px solid #eee; color: #666;">Nome</td><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>${nome} ${cognome}</strong></td></tr>
              <tr><td style="padding: 8px; border-bottom: 1px solid #eee; color: #666;">Email</td><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>${email}</strong></td></tr>
              <tr><td style="padding: 8px; border-bottom: 1px solid #eee; color: #666;">Telefono</td><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>${telefono || '—'}</strong></td></tr>
    ...
    // Rimetti il flag per il followup email
    await change.after.ref.update({ followupEmailPending: true });

    console.log(`✅ Re-iscrizione waitlist: ${nome} ${cognome} (${email}) - conferma inviata`);
    return null;
  });
...
/** Articoli italiani corretti per ciascuna professione.
 *  un  → articolo indeterminativo  (es. "sono {un_professione}" → "sono uno psicologo")
 *  del → articolo partitivo/specificativo (es. "lo studio {del_professione}" → "dello psicologo")
 */
...
export const sendBulkWaitlistEmail = functions
  ...
  .onCall(async (data, context) => {
    ...
    const replacePlaceholders = (template: string, recipient: any): string => {
      const token = Buffer.from(recipient.email || '').toString('base64url');
      const unsubscribeUrl = `https://tuaequipe.it/disiscrivi?t=${token}`;
      const invitePageUrl = `https://tuaequipe.it/invita?t=${token}`;
      const prof = recipient.professione || '';
      ...
      return template
        ...
        .replace(/\{referral_url\}/gi, invitePageUrl)
        .replace(/\{invite_page_url\}/gi, invitePageUrl);
    };
    ...
    for (let i = 0; i < recipients.length; i++) {
      ...
      const unsubscribeUrl = `https://tuaequipe.it/disiscrivi?t=${unsubscribeToken}`;
...
// ── Gruppi email waitlist ──

/**
 * Quando un nuovo utente si iscrive alla waitlist, lo assegna automaticamente
 * a tutti i gruppi con autoAssign=true e criteri soddisfatti.
 */
export const onWaitlistCreate = functions
  .region('europe-west1')
  .firestore
  .document('waitlist/{docId}')
  .onCreate(async (snapshot) => {
    ...
    try {
      const groupsSnap = await admin.firestore()
        .collection('waitlist_email_groups')
      ...
      for (const groupDoc of groupsSnap.docs) {
        ...
        // Check professione
        if (criteria.professioni && criteria.professioni.length > 0) {
          if (!criteria.professioni.includes(data.professione)) continue;
        }
        // Check città
        if (criteria.citta && criteria.citta.length > 0) {
          if (!criteria.citta.includes(data.citta)) continue;
        }
        // Giorni: un nuovo iscritto ha 0 giorni → solo gruppi senza daysMin/Max o con daysMin=0
        const daysMin = criteria.daysMin != null ? criteria.daysMin : 0;
        if (daysMin > 0) continue; // nuovo iscritto ha 0 giorni
        // Aggiunge l'id del doc waitlist al gruppo
        const currentMembers: string[] = group.memberIds || [];
        if (!currentMembers.includes(snapshot.id)) {
          await groupDoc.ref.update({
            memberIds: admin.firestore.FieldValue.arrayUnion(snapshot.id),
          });
        }
      }
    } catch (err) {
      console.error('❌ onWaitlistCreate: errore auto-assign gruppi:', err);
    }
    return null;
  });

/** Crea o aggiorna un gruppo email waitlist */
export const saveWaitlistEmailGroup = functions
  .region('europe-west1')
  .https
  .onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Non autenticato');
    ...
    const { id, name, description, criteria, autoAssign } = data;
    if (!name?.trim()) throw new functions.https.HttpsError('invalid-argument', 'Nome obbligatorio');
    ...
    if (id) {
      await admin.firestore().collection('waitlist_email_groups').doc(id).set(payload, { merge: true });
      return { id };
    } else {
      payload.createdAt = admin.firestore.FieldValue.serverTimestamp();
      payload.memberIds = [];
      const ref = await admin.firestore().collection('waitlist_email_groups').add(payload);
      return { id: ref.id };
    }
  });

/** Elimina un gruppo email waitlist */
export const deleteWaitlistEmailGroup = functions
  .region('europe-west1')
  .https
  .onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Non autenticato');
    ...
    const { id } = data;
    if (!id) throw new functions.https.HttpsError('invalid-argument', 'ID obbligatorio');
    await admin.firestore().collection('waitlist_email_groups').doc(id).delete();
    return { ok: true };
  });

/**
 * Ricalcola i memberIds di un gruppo in base ai criteri correnti.
 * Esegue una scansione completa della waitlist.
 */
export const rebuildWaitlistEmailGroup = functions
  .region('europe-west1')
  ...
  .onCall(async (data, context) => {
    ...
    const { id } = data;
    if (!id) throw new functions.https.HttpsError('invalid-argument', 'ID obbligatorio');
    const groupDoc = await admin.firestore().collection('waitlist_email_groups').doc(id).get();
    if (!groupDoc.exists) throw new functions.https.HttpsError('not-found', 'Gruppo non trovato');
    const group = groupDoc.data()!;
    const criteria = group.criteria || {};

    const waitlistSnap = await admin.firestore().collection('waitlist').get();
    const matchingIds: string[] = [];
    for (const wDoc of waitlistSnap.docs) {
      const w = wDoc.data();
      if (w.unsubscribed) continue;
      if (criteria.professioni && criteria.professioni.length > 0) {
        if (!criteria.professioni.includes(w.professione)) continue;
      }
      if (criteria.citta && criteria.citta.length > 0) {
        if (!criteria.citta.includes(w.citta)) continue;
      }
      if (criteria.daysMin != null || criteria.daysMax != null) {
        if (!w.createdAt?.toDate) continue;
        const days = Math.floor((Date.now() - w.createdAt.toDate().getTime()) / 86400000);
        if (criteria.daysMin != null && days < criteria.daysMin) continue;
        if (criteria.daysMax != null && days > criteria.daysMax) continue;
      }
      matchingIds.push(wDoc.id);
    }
    await groupDoc.ref.update({ memberIds: matchingIds, lastRebuiltAt: admin.firestore.FieldValue.serverTimestamp() });
    return { count: matchingIds.length };
  });

// ── Template email waitlist ──

export const saveWaitlistEmailTemplate = functions
  .region('europe-west1')
  .https
  .onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Non autenticato');
    ...
    const { id, name, subject, body } = data;
    if (!name?.trim() || !subject?.trim() || !body?.trim()) {
      throw new functions.https.HttpsError('invalid-argument', 'Nome, oggetto e corpo obbligatori');
    }
    const payload: Record<string, any> = {
      name: name.trim(),
    ...
    if (id) {
      await admin.firestore().collection('waitlist_email_templates').doc(id).set(payload, { merge: true });
      return { id };
    } else {
      payload.createdAt = admin.firestore.FieldValue.serverTimestamp();
      const ref = await admin.firestore().collection('waitlist_email_templates').add(payload);
      return { id: ref.id };
    }
  });

export const deleteWaitlistEmailTemplate = functions
  .region('europe-west1')
  .https
  .onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Non autenticato');
    ...
    const { id } = data;
    if (!id) throw new functions.https.HttpsError('invalid-argument', 'ID obbligatorio');
    await admin.firestore().collection('waitlist_email_templates').doc(id).delete();
    return { ok: true };
  });

// ── Disiscrizione dalla waitlist ──
export const unsubscribeFromWaitlist = functions
  .region('europe-west1')
  .https
  .onRequest(async (req, res) => {
    ...
    // Compatibile sia con link GET (?t=...) sia con vecchia chiamata callable (POST JSON)
    ...
    let email: string;
    try {
      // Nuovo formato: token base64url. Fallback: vecchio formato con email in chiaro.
      const decoded = Buffer.from(String(token), 'base64url').toString('utf8');
      email = (decoded && decoded.includes('@') && decoded.includes('.')) ? decoded : String(token);
    ...
    if (!snapshot.empty) {
      await snapshot.docs[0].ref.update({ unsubscribed: true });
      console.log(`✅ Disiscrizione waitlist: ${email}`);
    }
