...
export default function InvitesPage() {
  ...
  const handleRejectInvite = async (invite: InviteWithData) => {
    const confirmed = await showConfirm({ title: 'Rifiuta invito', message: 'Sei sicuro di voler rifiutare questo invito?', variant: 'warning', confirmText: 'Rifiuta' });
    if (!confirmed) return;
  ...
  const handleCancelInvite = async (invite: InviteWithData) => {
    const confirmed = await showConfirm({ title: 'Annulla invito', message: 'Sei sicuro di voler annullare questo invito?', variant: 'warning', confirmText: 'Annulla invito' });
    if (!confirmed) return;
