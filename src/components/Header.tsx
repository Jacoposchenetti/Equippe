...
export default function Header() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-white/95 text-slate-900 flex items-center justify-between px-4 z-20 shadow-sm border-b border-slate-200 backdrop-blur">
        <button onClick={toggle} className="p-2 rounded-xl hover:bg-slate-100 transition" aria-label="Menu">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>
        <Link to="/dashboard" className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-xl overflow-hidden bg-white p-1 shadow-sm ring-1 ring-slate-200">
            <img src="/logo_senza_scritta.png" alt="Logo" className="h-full w-full object-contain" />
          </div>
          <span className="font-bold text-sm">
            <span className="text-blue-700">tua</span><span className="text-green-700">equipe</span><span className="text-orange-600">.it</span>
          </span>
        </Link>
      ...
      {!isOpen && (
        <button
          onClick={toggle}
          className="hidden lg:flex fixed top-1/2 -translate-y-1/2 left-0 z-50 p-2 bg-white text-slate-700 rounded-r-xl shadow-lg ring-1 ring-slate-200 hover:bg-blue-50 hover:text-blue-700 transition"
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-white text-slate-900 z-40 flex flex-col shadow-2xl shadow-slate-900/10 border-r border-slate-200 transition-transform duration-300 ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
      {/* Logo + toggle button */}
      <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
        <Link to="/dashboard" className="flex items-center gap-2 hover:opacity-80 transition">
          <div className="h-9 w-9 rounded-xl overflow-hidden bg-white p-1 shadow-sm ring-1 ring-slate-200">
            <img src="/logo_senza_scritta.png" alt="tuaequipe.it Logo" className="h-full w-full object-contain" />
          </div>
          <span className="font-bold text-base">
            <span className="text-blue-700">tua</span>
            <span className="text-green-700">equipe</span>
            <span className="text-orange-600">.it</span>
          </span>
        </Link>
        <button
          onClick={toggle}
          className="p-1.5 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition"
      ...
      <div className="px-5 py-4 border-b border-slate-200 bg-slate-50/70">
        ...
        <div className="flex items-center gap-3 mb-2">
          {userProfile?.profile?.photoURL ? (
            <img src={userProfile.profile.photoURL} alt={userProfile.profile.nome} className="w-10 h-10 rounded-full object-cover border-2 border-white shadow-sm ring-2 ring-blue-100 shrink-0" />
          ) : (
            <div className="w-10 h-10 bg-blue-700 text-white rounded-full flex items-center justify-center font-bold text-sm shrink-0 shadow-sm">
              {getInitials()}
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto py-3 sidebar-nav">
          {navLinks.map((link) => {
            ...
            if (isDisabled) {
              return (
                <div key={link.href} className="mx-3 flex items-center gap-3 rounded-xl px-3 py-2.5 text-slate-400 opacity-60 cursor-not-allowed">
                  {navIcons[link.href]}
              ...
              {[
                ...
                <Link
                  ...
                  className={`mx-3 flex items-center rounded-xl px-3 py-2.5 text-sm transition-colors ${
                    pathname === link.href ? 'bg-amber-500 text-white' : 'text-amber-700 hover:bg-amber-50'
                  }`}
                >
                  {link.label}
                </Link>
        ...
        <div className="border-t border-slate-200 bg-slate-50/70 p-3 space-y-1">
          <Link
            to="/profile/edit"
            onClick={handleNavClick}
            className="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 hover:bg-white hover:text-blue-700 rounded-xl transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-xl transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
