...
export default function Header() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-gray-900 text-white flex items-center justify-between px-4 z-20 shadow-lg">
        <button onClick={toggle} className="p-2 rounded-lg hover:bg-gray-700 transition" aria-label="Menu">
      ...
      {!isOpen && (
        <button
          onClick={toggle}
          className="hidden lg:flex fixed top-1/2 -translate-y-1/2 left-0 z-50 p-2 bg-gray-900 text-white rounded-r-lg shadow-lg hover:bg-gray-700 transition"
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-gray-900 text-white z-40 flex flex-col shadow-2xl transition-transform duration-300 ${
      ...
      <div className="px-5 py-3 border-b border-gray-700">
        ...
        <div className="flex items-center gap-3 mb-2">
          {userProfile?.profile?.photoURL ? (
            ...
            <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center font-bold text-sm shrink-0">
              {getInitials()}
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto py-2 sidebar-nav">
          {navLinks.map((link) => {
            ...
            if (isDisabled) {
              return (
                <div key={link.href} className="flex items-center gap-3 px-5 py-3 text-gray-500 opacity-50 cursor-not-allowed">
                  {navIcons[link.href]}
          ...
          {isAdmin && (
            <div className="mt-3 border-t border-gray-700 pt-3">
              <p className="px-5 pb-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">Admin</p>
              {[
                ...
                <Link
                  ...
                  className={`flex items-center px-5 py-3 text-sm transition-colors ${
                    pathname === link.href ? 'bg-yellow-600 text-white' : 'text-yellow-400 hover:bg-gray-700'
                  }`}
                >
                  {link.label}
                </Link>
        ...
        <div className="border-t border-gray-700 p-3 space-y-1">
          <Link
            to="/profile/edit"
            onClick={handleNavClick}
            className="flex items-center gap-3 px-4 py-2 text-sm text-white hover:bg-gray-700 rounded-lg transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-400 hover:bg-gray-700 rounded-lg transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
