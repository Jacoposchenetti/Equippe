...
export default function MessagesPage() {
  ...
  const startNewConversation = async (otherUserId: string) => {
    ...
    try {
      ...
      // Verifica connessione prima di creare una nuova chat privata
      const connected = await isConnected(user.uid, otherUserId);
      if (!connected) {
        showToast('Devi essere connesso con questo utente per iniziare una chat. Vai al suo profilo e invia una richiesta di connessione.', 'warning');
        setCreatingConversation(false);
        return;
      }
