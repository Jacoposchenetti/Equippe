...
export default function EditProfilePage() {
  const { user, userProfile, refreshProfile, deleteCurrentUser, isAdmin, simulateUserView, setSimulateUserView } = useAuth();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-0 pb-24 sm:pt-4 sm:pb-8">
        ...
        <form className="space-y-6">
          ...
          {/* Sezione Admin: Vista Utente/Admin */}
          {isAdmin && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-yellow-800">Vista Admin / Utente</h2>
                  <p className="text-sm text-yellow-700 mt-1">
                    {simulateUserView
                      ? 'Stai vedendo l\'app come un utente normale. I link admin e le sezioni riservate sono nascosti.'
                      : 'Stai vedendo l\'app in modalità admin. Attiva la vista utente per simulare l\'esperienza di un utente normale.'}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setSimulateUserView(!simulateUserView)}
                  className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors focus:outline-none ${
                    simulateUserView ? 'bg-yellow-500' : 'bg-gray-300'
                  }`}
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${
                      simulateUserView ? 'translate-x-8' : 'translate-x-1'
                    }`}
                  />
                </button>
              </div>
              <p className="text-xs text-yellow-600 mt-3">
                {simulateUserView ? '👤 Vista utente attiva' : '🔑 Vista admin attiva'}
              </p>
            </div>
          )}
