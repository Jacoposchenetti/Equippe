...
import { doc, getDoc, collection, query, where, getDocs, addDoc, onSnapshot, Timestamp } from 'firebase/firestore';
...
import { getConnectionId } from '@/lib/connections';
...
export default function ProfilePage() {
  ...
  // Live-track connection status with this profile
  useEffect(() => {
    if (!currentUser || !uid) return;
    const connId = getConnectionId(currentUser.uid, uid);
    const unsub = onSnapshot(
      doc(db, 'connections', connId),
      (snap) => setIsConnectedToUser(snap.exists() && snap.data().status === 'accepted'),
      () => setIsConnectedToUser(false)
    );
    return () => unsub();
  }, [currentUser, uid]);

  // Fetch connection count for profile (two queries: userA and userB)
  useEffect(() => {
    if (!uid) return;
    let cancelled = false;
    const fetchCount = async () => {
      try {
        const [snapA, snapB] = await Promise.all([
          getDocs(query(collection(db, 'connections'), where('userA', '==', uid), where('status', '==', 'accepted'))),
          getDocs(query(collection(db, 'connections'), where('userB', '==', uid), where('status', '==', 'accepted'))),
        ]);
        if (!cancelled) setConnectionCount(snapA.size + snapB.size);
      } catch {
  ...
  const handleStartConversation = async () => {
    ...
    try {
      ...
      // Verifica connessione prima di aprire nuova chat privata
      if (!isConnectedToUser) {
        showToast('Devi essere connesso con questo professionista per iniziare una chat. Invia prima una richiesta di connessione.', 'warning');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-0 pb-24 sm:pt-4 sm:pb-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            ...
            {connectionCount !== null && (
              <div className="mt-3 flex items-center gap-1.5 text-sm text-gray-500">
                ...
                <span><strong>{connectionCount}</strong> connession{connectionCount === 1 ? 'e' : 'i'}</span>
              </div>
            )}
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full lg:w-auto">
              ...
              {canInteract && profileUser.profile.verificationInfo?.status === 'approved' && (
                <ConnectionButton
                  targetUid={uid}
                  targetName={profileUser.profile.nome}
                />
              )}
              ...
              {adminTeams.length > 0 && (
                (canInteract && profileUser.profile.verificationInfo?.status === 'approved') ? (
                  <button
                    ...
                    className="px-4 py-2 border border-gray-300 bg-white text-gray-700 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 text-sm font-medium whitespace-nowrap"
                  >
                    Invita in equipe
                  </button>
                ) : (
              ...
              {(canInteract && profileUser.profile.verificationInfo?.status === 'approved') ? (
                <button
                  ...
                  className="px-4 py-2 border border-gray-300 bg-white text-gray-700 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 text-sm font-medium whitespace-nowrap"
                >
