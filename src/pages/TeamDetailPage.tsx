...
import { autoConnectWithTeamMembers } from '@/lib/connections';
...
export default function TeamDetailPage() {
  ...
  const handleAcceptRequest = async (requestId: string, userId: string) => {
    try {
      ...
      // Auto-connetti il nuovo membro con tutti i membri esistenti
      const existingMemberIds = (team?.memberIds ?? []).filter((mid: string) => mid !== userId);
      if (existingMemberIds.length > 0) {
        await autoConnectWithTeamMembers(userId, existingMemberIds);
      }
