...
export interface Connection {
  id: string;           // "${smallerUid}_${largerUid}"
  userA: string;        // smaller UID (alphabetically)
  userB: string;        // larger UID
  requestedBy: string;  // UID who sent the request ('system' for auto-connections)
  status: ConnectionStatus;
  message?: string;     // optional message sent with the request
  createdAt: Timestamp;
  updatedAt: Timestamp;
}
...
export type NotificationType = 
  ...
  | 'connection_request'           // Richiesta di connessione ricevuta
...
export interface Notification {
  ...
  connectionId?: string;       // Per connection_request / connection_accepted
}
